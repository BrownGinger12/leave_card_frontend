...
Submit a leave application. Validates employee balance before inserting. Created with status `FOR HRMO ACTION`. **Balance is deducted immediately on submission.**
...
> For FL: two DEBIT entries are posted on submission — one from the FL balance and one from the VL balance.
...
> `with_pay`: optional, defaults to `true`
> `start_date` is used as `transaction_date` in the ledger so debits sort correctly after monthly credits
...
Get all leave applications for an employee in a given calendar year. Each application includes an embedded `ledger` array of all its associated transactions (DEBIT on submit, CREDIT reversal if returned/disapproved).
...
Get a paginated list of all leave applications across all employees, ordered by date filed descending.
...
> `status` must be one of: `FOR HRMO ACTION`, `FOR APPROVAL`, `RETURNED`, `DISAPPROVED`, `APPROVED`
...
{
  "statusCode": 400,
  "message": "Invalid status. Must be one of: FOR HRMO ACTION, FOR APPROVAL, RETURNED, DISAPPROVED, APPROVED"
}
```
...
Get a single leave application by its unique application number.
...
{
  "statusCode": 404,
  "message": "Leave application 'LA-A1B2C3D4' not found"
}
```
...
Submit an approval decision on a leave application. Valid for applications in `FOR HRMO ACTION` or `FOR APPROVAL` status. Cannot act on `APPROVED`, `RETURNED`, or `DISAPPROVED` applications.

**Balance behavior:**
| Decision | Ledger Effect |
|---|---|
| `FOR APPROVAL` | No ledger change — status updated only |
| `APPROVED` | No ledger change — balance was already deducted at submission |
| `RETURNED` | CREDIT reversal posted — balance fully restored |
| `DISAPPROVED` | CREDIT reversal posted — balance fully restored |

> For FL applications, `RETURNED`/`DISAPPROVED` posts two reversal CREDITs — one to FL balance and one to VL balance.
...
**Already finalized** `400`

```json
{
  "statusCode": 400,
  "message": "Leave application is already APPROVED and cannot be acted on"
}
```
...
```json
{ "statusCode": 400, "message": "query is required" }
```
...
Filter Special Orders by year and/or date range on `date_of_activity`. All filters optional and combinable. Results ordered by `date_of_activity` descending.
...
```json
{
  ...
  "page": 1,
  "limit": 10,
  "filters": { "year": 2026, "date_from": "2026-06-01", "date_to": "2026-06-30" },
  "data": [ ...special_orders ]
}
```
...
> **No approval step.** Balance is credited immediately on submission — a `CREDIT` is posted to the ledger (`source_type: SPECIAL_ORDER`) and the balance cache is updated automatically.

---

### POST `/service-credit-applications`

Submit a new service credit application. **Balance is credited immediately on submission** — there is no approval step. The credit type (`CTO` or `VSC`) is **auto-derived** from the employee's classification.
...
> `type` is **not accepted** — auto-derived from `employee_type` (TEACHING → VSC, NON_TEACHING → CTO)
> `participation_dates`: **required** — non-empty list of `YYYY-MM-DD` strings; the latest date determines `valid_until` for CTO
> `valid_until`: **auto-computed** — latest participation date + 1 year (CTO only); always `null` for VSC; not accepted as input
...
Get a single service credit application by its unique application number.
...
> `date_from` and `date_to` can be combined for a date range.
...
```json
{ "statusCode": 404, "message": "Special Order 1 not found" }
```
...
| Param | Required | Description |
|-------|----------|-------------|
| `q` | Yes | Matched against `application_number`, `employee_number`, `first_name`, `last_name` |
| `page` | No | Page number (default `1`) |
| `limit` | No | Records per page (default `10`) |
...
{
  "statusCode": 200,
  "special_order": { "id": 1, "special_order": "SO-2026-001", "activity_name": "Regional Year-End Assessment" },
  "query": "juan",
  "count": 2,
  "total": 2,
  "page": 1,
